  ...
  return (
    <>
      <button
        ...
        className="inline-flex w-full items-center justify-center rounded-full border border-[#ff6b5738] bg-[#7b2f26] px-5 py-3 text-sm font-semibold text-white transition hover:brightness-110"
      >
        Eliminar sesión
      </button>
          ...
          <div className="relative p-5 sm:p-6">
            ...
            <div className="pr-12">
              ...
              <p className="mt-3 text-sm leading-7 text-muted">
                Vas a borrar la sesión vinculada a{' '}
                <span className="font-semibold text-foreground">{sessionName}</span>. Esta acción es
                irreversible y actualizará las métricas agregadas del setup.
              </p>
            </div>

            <form action={deleteSessionAction} className="mt-6 space-y-4">
              <input type="hidden" name="sessionId" value={sessionId} />
              {returnTo ? <input type="hidden" name="returnTo" value={returnTo} /> : null}

              <div className="rounded-[1.35rem] border border-[#ff6b5733] bg-[#ff6b5710] px-4 py-3 text-sm text-[#ffb7aa]">
                Esta confirmación borrará la sesión inmediatamente.
              </div>
